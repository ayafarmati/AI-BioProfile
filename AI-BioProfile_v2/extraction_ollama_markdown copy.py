...
def _build_ollama_payload(cv_markdown: str, model_name: str) -> dict:
    ...
    json_template = """{
  "analyse_preliminaire": "Ton analyse globale ici...",
  ...
  "experiences": [
    {
      "titre_experience": "Titre du poste | Dates",
      "details_experience": [
        "Tâche accomplie 1...",
        "Tâche accomplie 2..."
      ]
  ...
  "autres_informations": "Téléphone, adresse, email, liens..."
}"""
...
def extract_cv_data_llm(file_bytes: bytes, model_name: str = "mistral", ollama_url: str = "http://localhost:11434/api/chat") -> str:
    """
...
def extract_cv_data_llm_from_text(cv_text: str, model_name: str = "mistral", ollama_url: str = "http://localhost:11434/api/chat") -> str:
    """
...
if __name__ == "__main__":
    ...
    # Mettre ici le chemin d'un fichier PDF pour tester le script directement
    test_pdf_path = "chemin_vers_un_cv_test.pdf"
    ...
    if os.path.exists(test_pdf_path):
        ...
        try:
            # Appel de l'orchestrateur (qui utilise 'mistral' par défaut)
            resultat_json = extract_cv_data_llm(pdf_bytes, model_name="mistral")
            print("\n=== RÉSULTAT DE L'EXTRACTION ===")
            print(resultat_json)
        except Exception as err:
            print(f"Erreur d'exécution : {err}", file=sys.stderr)
    else:
        print(f"[INFO] Script prêt. (Fournir un fichier PDF à {test_pdf_path} pour tester)")
